...
interface SecretNotificationConfig {
  id?: number;
  secret: number;
...
interface SecretNotificationsProps {
  secretId: number;
}

export default function SecretNotifications({ secretId }: SecretNotificationsProps) {
  const { authTokens } = useAuth();
  const { notify } = useToast();
  const baseURL = import.meta.env.VITE_API_URL;

  const [config, setConfig] = useState<SecretNotificationConfig>({
    secret: secretId,
  ...
  const fetchSecretConfig = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/notifications/secret-configs/for_secret/?secret_id=${secretId}`,
        {
      ...
      if (error.response?.status !== 404) {
        console.error("Failed to fetch secret notification config:", error);
      }
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios({
        method,
        url: `${baseURL}/notifications/secret-configs/for_secret/`,
        data: { ...config, secret_id: secretId },
        headers: { Authorization: `Bearer ${authTokens?.access}` },
      });

      notify("✅ Secret notification settings saved", "success");
      setConfig(response.data);
    } catch (error: any) {
          ...
          <div className="form-control">
            ...
            <label className="label py-0">
              <span className="label-text-alt text-xs">
                Enable custom notifications for this secret only
              </span>
            </label>
          </div>
